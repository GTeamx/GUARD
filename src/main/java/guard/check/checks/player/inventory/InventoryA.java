...
public class InventoryA extends GuardCheck {
    ...
    public void onPacket(PacketPlayReceiveEvent packet) {
        if (packet.getPacketId() == PacketType.Play.Client.WINDOW_CLICK) {
            ...
            else if(packet.getPlayer().isSneaking()) fail(packet, "Sneaked with an opened inventory", "NaN");
            else if(System.currentTimeMillis() - gp.lastAttack < 100) fail(packet, "Attacked with an opened inventory", "NaN");
            else removeBuffer();
        }
