...
@GuardCheckInfo(name = "Inventory B", category = GuardCategory.Player, state = GuardCheckState.STABLE, addBuffer = 1, removeBuffer = 0, maxBuffer = 10)
public class InventoryB extends GuardCheck {
    ...
    public void onMove(PacketPlayReceiveEvent packet, double motionX, double motionY, double motionZ, double lastMotionX, double lastMotionY, double lastMotionZ, float deltaYaw, float deltaPitch, float lastDeltaYaw, float lastDeltaPitch) {
        ...
        if(gp.getDistance(true) > 0) {
            if(inventoryState) fail(packet, "Moved with an opened inventory", "distance §9" + gp.getDistance(true));
