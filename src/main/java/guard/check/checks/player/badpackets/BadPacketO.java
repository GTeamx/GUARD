...
@GuardCheckInfo(name = "BadPacket O", category = GuardCategory.Player, state = GuardCheckState.STABLE, addBuffer = 0, removeBuffer = 0, maxBuffer = 0)
public class BadPacketO extends GuardCheck {
    ...
    public void onPacket(PacketPlayReceiveEvent packet) {
        if(packet.getPacketId() == PacketType.Play.Client.LOOK || packet.getPacketId() == PacketType.Play.Client.POSITION_LOOK) {
            ...
            if(wrapper.getPitch() > 90 || wrapper.getPitch() < -90) fail(packet, "Impossible pitch", "pitch §9" + wrapper.getPitch()); else removeBuffer();
        }
    }
