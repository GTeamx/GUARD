...
public class FlyE extends GuardCheck {
    ...
    public void onMove(PacketPlayReceiveEvent packet, double motionX, double motionY, double motionZ, double lastMotionX, double lastMotionY, double lastMotionZ, float deltaYaw, float deltaPitch, float lastDeltaYaw, float lastDeltaPitch) {
        ...
        if(motionY > .6 + (gp.player.hasPotionEffect(PotionEffectType.JUMP) ? ((gp.getPotionEffectAmplifier(PotionEffectType.JUMP) * 0.1) + 1) : 0) && !exempt) fail(packet, "Teleported upwards", "mY=" + motionY);
    }
