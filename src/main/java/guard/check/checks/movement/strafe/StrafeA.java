...
public class StrafeA extends Check {
    ...
    public void onMove(PacketPlayReceiveEvent packet, double motionX, double motionY, double motionZ, double lastmotionX, double lastmotionY, double lastmotionZ, float deltaYaw, float deltaPitch, float lastdeltaYaw, float lastdeltaPitch) {
        ...
        if(data.airticks > 3 && !exempt) {
            Angle = Math.abs(((Math.pow((motionX + motionZ), 8)) - (Math.pow(data.getMotionX(2) + data.getMotionZ(2), 8)) * 0.91));
            if(Angle >= 0.0000699) fail("Strafed in air", Angle);
        }
        if(Angle <= 0.0000699) removeBuffer();
