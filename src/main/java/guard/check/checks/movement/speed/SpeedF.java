...
@GuardCheckInfo(name = "Speed F", category = GuardCategory.Movement, state = GuardCheckState.Testing, addBuffer = 1, removeBuffer = 0.10, maxBuffer = 1)
public class SpeedF extends GuardCheck {
    ...
    public void onMove(PacketPlayReceiveEvent packet, double motionX, double motionY, double motionZ, double lastMotionX, double lastMotionY, double lastMotionZ, float deltaYaw, float deltaPitch, float lastDeltaYaw, float lastDeltaPitch) {
        ...
        if(!gp.onLowBlock && !exempt && (motionY - predictedMotion < -0.6456 || motionY - predictedMotion > 0.0386) && !gp.inAir && gp.onSolidGround) fail(packet, "YPort Motion"," res=" + (motionY - predictedMotion));
        else removeBuffer();
