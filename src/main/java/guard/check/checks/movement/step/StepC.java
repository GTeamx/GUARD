...
@GuardCheckInfo(name = "Step C", category = GuardCategory.Movement, state = GuardCheckState.Testing, addBuffer = 1, removeBuffer = 1, maxBuffer = 4)
public class StepC extends GuardCheck {
    ...
    public void onMove(PacketPlayReceiveEvent packet, double motionX, double motionY, double motionZ, double lastMotionX, double lastMotionY, double lastMotionZ, float deltaYaw, float deltaPitch, float lastDeltaYaw, float lastDeltaPitch) {
        ...
        if(lastMotionY - motionY < 0.01)  {
            if(!exempt && motionY != 0 && !gp.onLowBlock && motionY > -3.45) fail(packet, "Repeated motion", "lmY=" + lastMotionY + " mY=" + motionY);
        }
