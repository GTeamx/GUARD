...
@GuardCheckInfo(name = "FastClimb A", category = GuardCategory.Movement, state = GuardCheckState.STABLE, addBuffer = 1, removeBuffer = 1, maxBuffer = 4)
public class FastClimbA extends GuardCheck {
    ...
    public void onMove(PacketPlayReceiveEvent packet, double motionX, double motionY, double motionZ, double lastMotionX, double lastMotionY, double lastMotionZ, float deltaYaw, float deltaPitch, float lastDeltaYaw, float lastDeltaPitch) {
        ...
        if(motionY > 0.117600002289 && !gp.onLowBlock && !exempt && isExempt(ExemptType.CLIMBABLE)) fail(packet, "Climbing too fast", "mY §9" + motionY); else removeBuffer();
    }
